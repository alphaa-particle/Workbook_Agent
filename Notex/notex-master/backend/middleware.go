...
func init() {
	...
	if runtime.GOOS != "windows" {
		rotateOptions = append(rotateOptions, rotatelogs.WithLinkName("./logs/audit.log"))
	}

	writer, err := rotatelogs.New(logFiles, rotateOptions...)
	if err != nil {
...
func AuthMiddleware(secret string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Support JWT from cookie as well (browser clients)
		if tokenString == "" {
		...
		if tokenString == "" {
			// No JWT supplied: create/use a stable guest identity so the app works
			// end-to-end without mandatory OAuth login.
			c.Set("user_id", getOrCreateGuestUserID(c))
			c.Next()
			return
		}
...
func getOrCreateGuestUserID(c *gin.Context) string {
	const guestCookie = "notex_guest_id"

	if guestID, err := c.Cookie(guestCookie); err == nil && guestID != "" {
		return "guest:" + guestID
	}

	guestID := uuid.NewString()
	// Persist guest identity for one year.
	c.SetCookie(guestCookie, guestID, 365*24*60*60, "/", "", false, true)
