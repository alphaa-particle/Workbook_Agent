...
// sanitizeUserIDForPath replaces characters invalid in Windows paths (e.g. colon
// in "guest:<uuid>") so the user ID can safely be used as a directory name.
...
func (s *Server) loadNotebookVectorIndex(ctx context.Context, notebookID string) error {
	...
	for _, src := range sources {
		if src.Content != "" {
			if _, err := s.vectorStore.IngestSource(ctx, notebookID, src.ID, src.Name, src.Content); err != nil {
...
func (s *Server) handleHealth(c *gin.Context) {
	...
	if s.cfg.IsOllama() {
		llmModel = s.cfg.OllamaModel + " (Ollama)"
	}
...
func (s *Server) handleAddSource(c *gin.Context) {
	...
	if source.Content != "" {
		if chunkCount, err := s.vectorStore.IngestSource(ctx, notebookID, source.ID, source.Name, source.Content); err != nil {
...
func (s *Server) handleDeleteSource(c *gin.Context) {
	...
	if err := s.vectorStore.Delete(ctx, source.NotebookID, source.ID, source.Name); err != nil {
		golog.Errorf("failed to delete source from vector store: %v", err)
	}
...
func (s *Server) handleUpload(c *gin.Context) {
	...
	safeUserID := sanitizeUserIDForPath(userID)
	userUploadDir := fmt.Sprintf("./data/uploads/%s", safeUserID)
	...
	if source.Content != "" {
		if _, err := s.vectorStore.IngestSource(ctx, notebookID, source.ID, source.Name, source.Content); err != nil {
...
func (s *Server) handleServeFile(c *gin.Context) {
	...
	filePath := filepath.Join("./data/uploads", sanitizeUserIDForPath(ownerUserID), filename)
...
// handleListPublicNotebooks lists all public notebooks
