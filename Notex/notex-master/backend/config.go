...
func LoadConfig() Config {
	...
	// Prefer Ollama when explicitly configured and no OpenAI API key is provided.
	if cfg.OpenAIBaseURL == "" && cfg.OllamaBaseURL != "" && cfg.OpenAIAPIKey == "" {
		cfg.OpenAIBaseURL = cfg.OllamaBaseURL
	}

	// Auto-detect provider from model name as a fallback.
...
func ValidateConfig(cfg Config) error {
	...
	hasOllama := cfg.OllamaBaseURL != "" || (cfg.OpenAIBaseURL != "" && contains(cfg.OpenAIBaseURL, "11434"))
...
func (c *Config) IsOllama() bool {
	if c.OllamaBaseURL != "" && c.OpenAIAPIKey == "" {
		return true
	}
