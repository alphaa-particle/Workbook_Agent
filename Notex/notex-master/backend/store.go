...
func (s *Store) ensureUserExists(ctx context.Context, userID string) error {
	...
	err := s.db.QueryRowContext(ctx, `SELECT id FROM users WHERE id = ?`, userID).Scan(&existingID)
	if err == nil {
		return nil
	}
	...
	now := time.Now().Unix()
	emailSafeID := strings.NewReplacer(":", "_", " ", "_", "@", "_at_").Replace(userID)
	email := fmt.Sprintf("%s@local.notex", emailSafeID)

	_, err = s.db.ExecContext(ctx, `
		INSERT OR IGNORE INTO users (id, email, name, avatar_url, provider, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?)
	`, userID, email, userID, "", "guest", now, now)
...
// ListPublicNotebooks lists all public notebooks
